...
// runtime
    ...
    $.get(url, (data) => {
        ...
        if (!Plot.plots.length) {
            ...
            // XXXXX european_countries
            for (let country of ["Czechia", "United Kingdom"]) {
...
function refresh(event = null) {
    ...
    if (event === true) {
        ...
        // if we are not just come here for the first time (loaded from hash),
        //  we may want to store the changes to hash
